...
#### To generate the PDF factsheets, from the root directory run the following:  *$ sh Script/run_scripts.sh username key* where _username_ and _key_ are your username and API key from pdfcrowd, the application which converts html documents to pdf. In run_scripts.sh, the following scripts are run in sequence:
